...
class TransactionListVente extends StatefulWidget {
  ...
  @override
  State<TransactionListVente> createState() => _TransactionListVenteState();
}

class _TransactionListVenteState extends State<TransactionListVente> {
  ...
  Widget build(BuildContext context) {
    final transactions = Provider.of<TransactionProvider>(context)
        .transactions
        .where((tx) => tx.type == 'vente' && _dateFilterHelper.matchDate(tx.date))
        .toList()
      ..sort((a, b) => b.date.compareTo(a.date));

    final clients = Provider.of<ClientProvider>(context).clients;

    // Filtrage par searchQuery et selectedFilter
    final filteredTransactions = transactions.where((tx) {
      ...
      if (widget.selectedFilter == 'Nom') {
        return client.nom.toLowerCase().contains(widget.searchQuery.toLowerCase());
