...
class _RestoreBackupScreenState extends State<RestoreBackupScreen> {
  ...
  Future<void> _restoreLocal(File file) async {
    ...
    final mode = await _askRestoreMode(currentContext);
    if (mode == null || !currentContext.mounted) return;

    await executeWithLoading(currentContext, () async {
      ...
      ScaffoldMessenger.of(currentContext).showSnackBar(
        const SnackBar(content: Text('✅ Données restaurées. Relancez l\'app svp!')),
      );
    });
  }
  ...
  Future<void> _restoreDrive(drive.File file) async {
    ...
    await executeWithLoading(currentContext, () async {
      ...
      if (downloaded != null) {
        ...
        ScaffoldMessenger.of(currentContext).showSnackBar(
          const SnackBar(content: Text('❌ Téléchargement échoué.')),
        );
      }
    });
  }
