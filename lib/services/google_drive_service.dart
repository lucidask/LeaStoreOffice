...
class GoogleDriveService {
  ...
  static Future<void> supprimerBackupsDriveAnciens({int jours = 30}) async {
    try {
      final driveApi = await _getDriveApi();
      final cutoff = DateTime.now().subtract(Duration(days: jours));

      // 1. Lister les fichiers avec "backup_" dans le nom
      final files = await driveApi.files.list(
        q: "name contains 'backup_' and trashed = false",
        $fields: "files(id, name, createdTime)",
      );

      final fichiers = files.files ?? [];

      for (final file in fichiers) {
        if (file.createdTime != null && file.id != null) {
          final created = file.createdTime!;
          if (created.isBefore(cutoff)) {
            debugPrint("🗑️ Suppression du fichier ${file.name} (créé le $created)");
            await deleteFileById(file.id!);
          }
        }
      }
    } catch (e) {
      debugPrint("⚠️ Erreur lors de la suppression des anciens backups Drive : $e");
    }
  }
