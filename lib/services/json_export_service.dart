...
class JsonExportService {
  ...
  static Future<void> supprimerAnciensBackups({int jours = 30}) async {
    final dir = await getExternalStorageDirectory();
    final backupDir = Directory('${dir!.path}/backup');
    ...
    try {
      final fichiers = backupDir.listSync();

      for (final f in fichiers) {
        if (f is File && f.path.endsWith('.json')) {
          final stat = await f.stat();
          final age = maintenant.difference(stat.modified).inDays;

          if (age >= jours) {
            await f.delete();
            debugPrint('🗑️ Supprimé : ${f.path}');
          }
        }
      }
    } catch (e) {
      debugPrint('⚠️ Erreur pendant la suppression automatique des backups : $e');
    }
  }
